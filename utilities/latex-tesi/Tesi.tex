...
\begin{itemize}
\item \textbf{Google}: \textit{``status": ``Failed"} al primo messaggio di risposta (initiateAuthenticationResponse), con \textit{``subjectCode: ``8.8.2"}, \textit{``reasonCode": ``3.1"}, \textit{``message: Execution Failed} $\rightarrow$ ``none of the proposed Public Key Identifiers is supported by the SM-DP+.
\item \textbf{Infineon}:
\item \textbf{GSMA}:
