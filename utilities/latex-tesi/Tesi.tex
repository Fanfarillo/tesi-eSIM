...
\subsection{Condizioni iniziali}
\begin{itemize}
\item L'applicazione LPA ha recuperato l'indirizzo del server SM-DP+ e opzionalmente l'identificativo della chiave pubblica del CI consentita; se tale identificativo viene effettivamente recuperato dall'eUICC, allora l'LPA deve restringere l'insieme degli identificativi delle chiavi pubbliche dei CI compatibili a quel valore.
...
\item Se la funzione handleNotification() è configurata nel Service Provider, il server SM-DP+ la invoca per notificare il Service Provider riguardo la richiesta del cambio di dispositivo.
...
\item L'LPA del vecchio dispositivo invoca la funzione prepareDeviceChange() passandovi come parametri smdpSigned4, smdpSignature4 ed eventualmente l'hash del codice di conferma. Quest'ultimo viene calcolato come SHA256(SHA256(ConfirmationCode) | transactionID).
\item L'LPA del vecchio dispositivo invoca la funzione confirmDeviceChange() passandovi come parametri transactionID e prepareDeviceChangeResponse.
...
\item Se il booleano isNewProfileRequired vale FALSE, allora il server SM-DP+ prepara il profilo per il download. Inoltre, se nello step (5) è stato fornito un EID, allora il server lo collega col download del profilo preparato. Infine, il server invoca la funzione handleNotification() per notificare il Service Provider riguardo l'esito della preparazione del profilo.
