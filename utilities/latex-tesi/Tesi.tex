...
\noindent Dalla tabella \ref{tab:IP-addr} è possibile concludere che non c'è un pattern vero e proprio negli indirizzi IP dei server SM-DP+, per cui è impensabile ricavare ulteriori indirizzi IP a partire da quelli noti. Inoltre, è possibile osservare che gli operatori Iliad e Very Mobile si appoggiano sul medesimo server SM-DP+.\\
...
\hline
eSIM-discovery & Server SM-DS & Verrà contattato usando una certificate chain custom con CI = GSMA*.\\
\hline
GSMA & Server SM-DS & Verrà contattato usando una certificate chain custom con CI = GSMA*.\\
\hline
OTG-eUICC & Server SM-DS & Verrà contattato usando una certificate chain custom con CI = GSMA*.\\
\hline
Google & Server SM-DP+ di test & Verrà contattato usando una certificate chain di test**. Matching ID: 052X-UFXS-CQIY-PNGL.\\
\hline
...
\hline
Truphone & Server SM-DP+ di test & Verrà contattato usando una certificate chain di test. Matching ID: QR-G-5C-1LS-1W1Z9P7.\\
\hline
...
\textit{**La certificate chain di test supportata dal server di Google è diversa da quella prevista dagli altri server di test.}
...
\item \textbf{OTG-eUICC}:
\item \textbf{Google}: \textit{``status": ``Failed"} al secondo messaggio di risposta (authenticateClientResponse), con \textit{``subjectCode: ``8.11.1"}, \textit{``reasonCode": ``3.9"}, \textit{``message": ``Execution Failed"} $\rightarrow$ chiave pubblica del CI sconosciuta: il CI utilizzato dal certificato dell'EUM non è una Root trusted per il server SM-DP+.
\item \textbf{Infineon}: n.d.
\item \textbf{Sysmocom}:
\item \textbf{Truphone}:
...
\begin{itemize}
\item \textbf{Google}: HTTP\_INTERNAL\_SERVER\_ERROR (status code 500) al secondo messaggio di risposta (getBoundProfilePackageResponse).
\item \textbf{Infineon}: n.d.
...
\item \textbf{Tim}: \textit{``status": ``Failed"} al primo messaggio di risposta (getBoundProfilePackageResponse), con \textit{``subjectCode: ``8.10.1"}, \textit{``reasonCode": ``3.9"}, \textit{``subjectIdentifier": ``Transaction ID"}, \textit{``message": ``Unknown"}.
\item \textbf{Gemello di Tim}: \textit{``status": ``Failed"} al primo messaggio di risposta (getBoundProfilePackageResponse), con \textit{``subjectCode: ``8.10.1"}, \textit{``reasonCode": ``3.9"}, \textit{``subjectIdentifier": ``Transaction ID"}, \textit{``message": ``Unknown"}.
\item \textbf{Osmo-smdpp}: H\textit{``status": ``Failed"} al primo messaggio di risposta (getBoundProfilePackageResponse), con \textit{``subjectCode: ``8.10.1"}, \textit{``reasonCode": ``3.9"}, \textit{``message": ``The RSP session identified by the TransactionID is unknown"}.
...
\noindent A proposito di certificati, in rete sono disponibili quelli delle RootCA incontrate finora, ovvero GSM Association (i.e. GSMA), RSPTEST (a capo della certificate chain di test usata da Infineon e Osmo-smdpp) e Symantec Corporation (a capo della certificate chain di test usata da Google). Sempre con l'ausilio del sito \textit{ASN.1 JavaScript decoder}, è stato possibile ispezionare anche questi altri due certificati.
